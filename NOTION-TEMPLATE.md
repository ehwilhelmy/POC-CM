...
> We've built interactive prototypes addressing the top caretaker pain points after the Auth0 migration. Please review the flows below and leave your feedback — gut reactions, things that feel off, and anything that doesn't match what you're hearing from camps.
...
## Flow 1: New Caretaker Auth Flow
...
## Flow 2: Returning Caretaker Auth Flow
...
| **Proposed Journey** | Primary caretaker invites a guest from their dashboard → guest receives email with personal note → creates account → lands in limited portal. |
...
| **Proposed Journey** | Caretaker opens the Campanion mobile app → logs in → one login, all their camps — Campanion branding gives multi-camp context. |
...
| Account Lookup | Camp staff tool to search caretaker email and see account status | [link] |
| Expired Link | What caretakers see when a verification or reset link has expired | [link] |
...
1. **Branding** — Does the camp-branded login feel trustworthy? Would this reduce the "where am I?" calls from caretakers?
2. **Guest flow** — Does the personal note from the primary caretaker make the guest invite feel less like spam?
